...
export const RevenueContextProvider: React.FC<{ children: React.ReactNode }> = (
  props
) => {
  ...
  useEffect(() => {
    const setUp = async (
      ...
      let globCData;
      try {
        const cData = await getContractData(
          connection,
          new PublicKey(REV_SHARE_DATA_ACCOUNT)
        );
      ...
      try {
        ...
        if (globCData) {
          if (
            tokenBalance <
            formatAmount(
              globCData.minimumTokenBalanceForClaim,
              REV_SHARE_TOKEN_DECIMALS
            )
          ) {
            setError(
              "You're not eligible to claim due to insufficient balance"
            );
            setPoolShare("0");
          } else {
