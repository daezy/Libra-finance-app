...
export const getContractData = async (
  ...
  if (!info.owner.equals(new PublicKey(REV_SHARE_PROGRAM_ID)))
    throw new Error("TokenInvalidAccountOwnerError");
  if (info.data.length != ContractDataAccountLayout.span)
    throw new Error("TokenInvalidAccountSizeError");
  ...
  return {
    ...
    depositPerPeriod: parseInt(
      (data.depositPerPeriod as unknown as bigint).toString()
    ),
    minimumTokenBalanceForClaim: parseInt(
      (data.minimumTokenBalanceForClaim as unknown as bigint).toString()
    ),
  };
};
...
export const getUserData = async (
  ...
  if (info.data.length != UserDataLayout.span)
    throw new Error("TokenInvalidAccountSizeError");
...
export const calculateShare = async (
  ...
  if (info && contractData) {
    ...
    return (
      (tokenBalance * contractData.depositPerPeriod) /
      formatAmount(parseInt(data.supply.toString()), REV_SHARE_TOKEN_DECIMALS) /
      LAMPORTS_PER_SOL
    );
  }
  return 0;
...
export const initializeContract = async (
  ...
  const createDataAcctIX = SystemProgram.createAccount({
    ...
    lamports: await connection.getMinimumBalanceForRentExemption(
      ContractDataAccountLayout.span
    ),
    fromPubkey: payer,
  ...
  const initIX = new TransactionInstruction({
    ...
    keys: [
      ...
      {
        pubkey: dataAccountKeypair.publicKey,
        isSigner: false,
        isWritable: true,
      },
...
export const handleClaim = async (
  ...
  const instructionData = Buffer.from(Uint8Array.of(1));
  const claimIX = new TransactionInstruction({
    ...
    keys: [
      ...
      { pubkey: tokenMint, isSigner: false, isWritable: false },
      {
        pubkey: new PublicKey(REV_SHARE_DATA_ACCOUNT),
        isSigner: false,
        isWritable: false,
      },
      {
        pubkey: new PublicKey(REV_SHARE_PDA_ADDRESS),
        isSigner: false,
        isWritable: true,
      },
      { pubkey: SystemProgram.programId, isSigner: false, isWritable: false },
